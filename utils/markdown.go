...
func HTML(input string) template.HTML {
	...
	result := re.ReplaceAllStringFunc(string(htmlContent), func(matches string) string {
		...
		count := len(lines)
		if count == 1 {
			return fmt.Sprintf("<p>%s</p>", matches)
		}

		for _, data := range lines {
			images += fmt.Sprintf(`<div class="col-md-%d mb-4"><div class="card bg-dark">%s</div></div>`,
				func() int {
					if count == 2 || count == 4 {
						return 6
					}
		...
		return fmt.Sprintf(`<div class="row">%s</div>`, images)
	})
